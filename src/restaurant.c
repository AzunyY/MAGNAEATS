...
*/
int execute_restaurant(int rest_id, struct communication_buffers* buffers,
...
*/
void restaurant_receive_operation(struct operation* op, int rest_id, struct
  communication_buffers* buffers, struct main_data* data, struct semaphores* sems)
{
    if(*(data->terminate))
      return;
...
*/
void restaurant_process_operation(struct operation* op, int rest_id, struct
...
* depois de escrever.
*/
void restaurant_forward_operation(struct operation* op, struct
  communication_buffers* buffers, struct main_data* data, struct semaphores* sems)
{

  produce_begin(sems -> rest_driv); //o processo espera que haja dados para ler
  write_rest_driver_buffer(buffers->rest_driv, data->buffers_size, op);
  produce_end(sems -> rest_driv); // o processo notifica o produtor que acabou de ler, i.e., tem uma slot vazio
